# From https://github.com/tensorflow/models/blob/master/research/object_detection/dataset_tools/create_pascal_tf_record.py

# Usage : python create_tfrecords.py --image_dir=images --annotations_dir=annotations --label_map_path=label_map.pbtxt --output_path=data.tfrecord
...
def dict_to_tf_example(data, image_dir, label_map_dict):
    ...
    if not data['filename'].endswith('.png'):
        full_path = os.path.join(image_dir, '_'.join(json.loads(data['filename'].replace("'", '"'))))
    else:
